...
(function() {
  ...
  SVGcompiler = (function() {
    function SVGcompiler() {
      ...
      this.dummyDoc.close(SaveOptions.DONOTSAVECHANGES);
    }
    ...
    SVGcompiler.prototype.copyGroupedItemsToDummyDoc = function() {
      ...
      for (i = 0, len = ref.length; i < len; i++) {
        ...
        this.exportMiniSvg(item.name, width, height);
        results.push(newItem.remove());
      }
      return results;
    };

    SVGcompiler.prototype.exportMiniSvg = function(name, width, height) {
      var dest, exportOptions, fileSpec, folder, format, options, type;
      dest = this.sourceDoc.path + "/compiled/" + name;
      format = "SVG";
      options = [];
      ...
      this.dummyDoc.artboards[0].artboardRect = this.dummyDoc.pageItems[0].visibleBounds;
